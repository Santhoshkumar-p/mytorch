...
async def stream_openai(
    ...
    # Thinking / reasoning effort (custom budgets are not a direct OpenAI concept; effort string used)
    ...
    # session_id → passed as user field for cache-aware routing
    if config.session_id:
        params["user"] = config.session_id

    # on_payload hook
    if config.on_payload:
        override = await config.on_payload(params, model_config)
    ...
    try:
        ...
        # on_response: OpenAI SDK doesn't expose raw headers easily; fire with status 200 once stream opens
        if config.on_response:
            await config.on_response(200, {}, model_config)
