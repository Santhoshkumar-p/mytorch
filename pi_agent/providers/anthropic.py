...
async def stream_anthropic(
    ...
    # Use custom per-level budget if provided, else built-in defaults
    ...
    # session_id → forwarded as a metadata hint (Anthropic uses user_id for routing)
    if config.session_id:
        body["metadata"] = {"user_id": config.session_id}

    # on_payload hook — caller can inspect or replace the request body
    if config.on_payload:
        override = await config.on_payload(body, model_config)
        ...
        async with httpx.AsyncClient() as client:
            async with client.stream(
                ...
                if config.on_response:
                    await config.on_response(resp.status_code, dict(resp.headers), model_config)
