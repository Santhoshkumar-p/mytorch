    ...
    def forward(self, x, h_0=None):
        ...
        for seq in range(seq_len):
            for l in range(len(self.rnn)):
                if l == 0:
                    state = self.rnn[l](x[:,seq,:], hidden[l])
                else:
        ...
        * Normalize dh by batch_size since initial hidden states are also treated
        ...
        for seq in reversed(range(seq_len)):
            for l in reversed(range(self.num_layers)):
                h_prev_l = self.hiddens[seq+1][l-1] if l != 0 else self.x[:, seq, :]
                h_prev_t = self.hiddens[seq][l]
                h_t = self.hiddens[seq+1][l]
                dx, dh[l] = self.rnn[l].backward(dh[l], h_t, h_prev_l, h_prev_t)
                if l != 0:
                    dh[l-1] = dx if seq == (seq_len - 1) else (dh[l-1] + dx)
                    #dh[l] = dh[l-1]+dx
